...
function loadMusic(indexNumb){
  ...
  musicName.innerText = songList[indexNumb - 1];
  musicImg.src = `${imgSrc}`;
  mainAudio.src = songSrc +'/'+ songList[indexNumb - 1] ;
...
for (let i = 0; i < songList.length; i++) {
  //let's pass the song name, artist from the array
     let liTag = `<li li-index="${i + 1}">
                 <div class="row">
                   <span>${songList[i]}</span>
                 </div>
                <audio class="${songList[i]}" src="${songSrc}/${songList[i]}"></audio>
                <span id="${songList[i].src}" class="audio-duration">3:40</span>
               </li>`;
